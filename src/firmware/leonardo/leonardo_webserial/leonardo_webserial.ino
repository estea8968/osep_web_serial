...
char* serialString()
{
  ...
  static char str[161]; // For strings of max length=20
...
void loop() 
{
  static boolean needPrompt=true;
    ...
    if(strcmp(commandString, "ws") == 0){
        ...
        char* sp = "";
        int count;
        for (count = 0; inputTime[count] != '\0'; ++count);
        for( int i = 0; i<count-1 ; i++){
          ...
          pixels.setPixelColor(atoi(sp), r, g, b);
        }
        Serial.println(inputPin);
        pixels.setPixelColor(2,50,50,50);
        pixels.show(); 
      }
