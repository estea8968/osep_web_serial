...
const FormSonarRead = {
    ...
    'en': 'Read SONAR  Echo [ECHO_PIN] Trig [TRIGGER_PIN] ',
...
const FormWs2812SetPin ={
    'en': "Led array on pin:[PIN] ",
    'zh-tw': "設定Led陣列在腳位[PIN]",
};
const FormWs2812SetNum = {
    'en': 'Led array light[NUM],color:[RGB],value:[VALUE]',
    'zh-tw': 'led陣列，亮第[NUM]顆，顏色:[RGB],值:[VALUE]',
};
...
const FormRGB ={
    'en': ['red','green','blue','yellow','cyan','white'],
    'zh-tw': ['紅','綠','藍','黃','青','紫','白'],
};
...
class Scratch3ArduinoWebSerial {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    arguments: {
                        ...
                        TH: {
                            type: ArgumentType.STRING,
                            defaultValue: MENU_DHT_SENSORS[the_locale][0],
                            menu: 'dht_items'
                        }
                    }
                },
    ...
    async tone_on(args,util) {
        ...
        // make sure duration maximum is 10 seconds
        if (duration > 10000) {
            duration = 10000;
        }
        ...
        console.log('sendData:',sendData);
        this.serialSend(sendData);
        //this.await(util,duration);
    }
 
    async await (util,duration) {
        if (util.stackTimerNeedsInit()) {
            //const duration = Math.max(0, Cast.toNumber(args.DURATION));
            const duration = Math.max(0, Cast.toNumber(duration));
    ...
    ws2812_set_pin(args) {
        ...
        //ws2812_pin = args['PIN'].toString();
//        ws2812_set = args['RGB'];
        
        /*for(i=0;i<FormRGB[the_locale].length;i++){
            if(FormRGB[the_locale][i] == ws2812_set){
                send_color_data = send_rgb[i]+'#';
            }
        }*/
        //return ws2812_pin+','+send_color;
    }
    ...
    ws2812_set_num(args){
        ...
        send_color_data = send_color_data +led_num+','+color_set_num.toString()+','+led_value+',';
