    ...
    parseAttrFromLASS(args) {
        ...
        if (this.isDataFetched()) {
            try {
                ...
                let data = parsed[attr];
                //console.log(attr);
                console.log(new Date(data).getHours());
                const hour=new Date(data).getHours();
                const minute = new Date(data).getMinutes();
                const second =new Date(data).getSeconds();
                if(attr =='timestamp'){
                    const data_ary = data.split('T');
                    data = data_ary[0]+' '+hour+':'+minute+':'+second;
                }
            ...
            if (res.ok) {
                res.json().then(json => {
                    ...
                    adata.forEach(function(adata, index) {
                        let sn = JSON.stringify(adata.sitename);
                        sn = sn.replaceAll('"','');
                        aSN_array.push(sn);
                        //console.log(SiteName_array);
                      });
