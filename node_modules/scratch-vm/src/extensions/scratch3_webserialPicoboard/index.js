...
class Scratch3PicoboardWebSerial {
        ...
        return {
            ...
            blocks: [
                ...
                /*
                {
                    opcode: 'analog_0',
                    blockType: BlockType.REPORTER,
                    text: FormAnalog_0[the_locale], 
                },
                {
                    opcode: 'analog_6',
                    blockType: BlockType.REPORTER,
                    text: FormAnalog_6[the_locale],
                },
                {
                    opcode: 'analog_3',
                    blockType: BlockType.REPORTER,
                    text: FormAnalog_3[the_locale],
                },
                {
                    opcode: 'digital_2',
                    blockType: BlockType.REPORTER,
                    text: FormDigital_2[the_locale],
                },
                {
                    opcode: 'analog_5',
                    blockType: BlockType.REPORTER,
                    text: 'A',//FormAnalog_3[the_locale],
                },
                {
                    opcode: 'analog_4',
                    blockType: BlockType.REPORTER,
                    text: 'B',//FormAnalog_3[the_locale],
                },
                {
                    opcode: 'analog_2',
                    blockType: BlockType.REPORTER,
                    text: 'C',//FormAnalog_3[the_locale],
                },
                {
                    opcode: 'analog_1',
                    blockType: BlockType.REPORTER,
                    text: 'D',//FormAnalog_3[the_locale],
                },*/
            ],
            menus: {
    ...
    async writeSerial(send_value){
        let sendData = parseInt(send_value,10);
        const encoder = new TextEncoder();
        let writer = p_port.writable.getWriter();
        await writer.write(encoder.encode(sendData));
        writer.releaseLock();
        await this.readSerial(send_value);
    }

    async readSerial(send_value){
        ...
        try{
            ...
            //搖桿 lastDataSample[7] J4 A1 lastDataSample[0] J3 A2 lastDataSample[1] 
            //聲音 lastDataSample[6] J2 A4 lastDataSample[2] J1 A5 lastDataSample[5] 光敏A6 lastDataSample[5] 按鈕 d2 lastDataSample[3]
            const last_ary = [7,0,1,6,2,4,5,3];
            if(sp_ary[0]==='A' || sp_ary[0]==='D'){
                if(!!sp_ary[1] ){
                    lastDataSample[last_ary[send_value]]=Number(sp_ary[1]);
                }else{
                    reader.releaseLock();
                    this.readSerial(send_value);
                }
            }
        }catch (error) {
            console.log(error);
  ...
  async current_sensor_value(args) {
    ...
    const last_ary = [7,0,1,6,2,4,5,3];
    const sendKey_ary = [0,6,3,7,5,4,2,1];
    //console.log('sendKey_ary:',sendKey_ary[map_key]);
    await this.writeSerial(sendKey_ary[item_index]);
    console.log('map_key=',map_key);
    // retrieve the data from the last data sample
    //回傳lastDataSample的值
    console.log('lastDataSample map_key=',lastDataSample[last_ary[map_key]]);
    //console.log(lastDataSample);
  ...
  /*
    async analog_0(){
        ...
        //搖桿A0 lastDataSample[7]
        await this.writeSerial(0);
        //await this.readSerial(0);
        return lastDataSample[7];
    }

    async analog_1(){
        //J4 A1 lastDataSample[0]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(1);
        return lastDataSample[0];
    }

    async analog_2(){
        //J3 A2 lastDataSample[1]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(2);
        return lastDataSample[1];
    }

    async analog_3(){
        //soundA3 lastDataSample[2]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(3);
        return lastDataSample[6];
    }

    async analog_4(){
        //J2 A4 lastDataSample[0]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(4);
        return lastDataSample[2];
    }

    async analog_5(){
        //J1 A5 lastDataSample[0]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(5);
        return lastDataSample[4];
    }

    async analog_6(){
        //光敏A6 lastDataSample[1]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(6);
        return lastDataSample[5];
    }

    async digital_2(){
        //button d2 lastDataSample[3]
        await new Promise(resolve => setTimeout(resolve, 50));
        await this.writeSerial(7);
        return lastDataSample[3];
    }
    */
    ...
    async range_convert(args) {
            ...
            await this.writeSerial(send_aa[item_index]);
            //await this.readSerial(send_aa[item_index]);
            map_key = this.mapAllSensors(item_index);
            console.log(map_key);
