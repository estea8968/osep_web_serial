...
//["滑桿", "光線", "聲音", "按鈕", "A", "B", "C", "D"],
const send_key_array = [0,6,3,7,5,4,2,1];
...
class Scratch3PicoboardWebSerial {
    ...
    async readSerial(){
        ...
        try{
            ...
            if(sp_ary[2]=='p'){
                var i = Number(sp_ary[0].substring(1, 2));
                if(i==7){
                    //按鈕
                    if(sp_ary[1] == '1'){
                        lastDataSample[7]=0;
  ...
  async is_button_pressed(args) {
    ...
    await new Promise(resolve => setTimeout(resolve, 10));
    this.readSerial();
    return Boolean(Number(lastDataSample[7]));
    }
  ...
  async current_sensor_value(args) {
    ...
    await this.writeSerial(send_key);
